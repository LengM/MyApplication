...
import com.yng.ming.myapplication.util.video.CallEvent;
...
/**
 * Created by lzan13 on 2016/10/18. 视频通话界面处理
 */
public class VideoCallActivity extends CallActivity {
    ...
    @Bind(R.id.layout_surface_container)
    ...
    @Bind(R.id.layout_call_control)
    RelativeLayout layout_call_control;
    @Bind(R.id.local_btn_camera_switch)
    ImageButton local_btn_camera_switch;
     ...
     * 界面控件点击监听器
     */
    @OnClick({R.id.fab_reject_call, R.id.fab_end_call, R.id.fab_answer_call})
    void onClick(View v) {
        switch (v.getId()) {
...
//             break;
    ...
    /**
     * 接通通话，修改本地UI在右侧，远程在左侧
     */
    private void onCallSurface() {
        // 更新通话界面控件状态
        surfaceState = 0;

        int width = layout_call_control.getWidth() / 2;
        int height = layout_call_control.getHeight();

        localParams = new RelativeLayout.LayoutParams(width, height);
        localParams.width = width;
        localParams.height = height;
        localParams.leftMargin = width + 1;
        localParams.addRule(RelativeLayout.ALIGN_PARENT_RIGHT);
        localSurface.setLayoutParams(localParams);

        oppositeParams = new RelativeLayout.LayoutParams(width, height);
        oppositeParams.width = width;
        oppositeParams.height = height;
        oppositeParams.rightMargin = width + 1;
